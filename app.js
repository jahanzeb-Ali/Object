...
let shadowCon = document.querySelector(".container-shadow")
...
for (const item of UsersObj) {
    ...
    <button id="${count}" onclick="showBox('${count}')">Add to cart</button>
</div>`
}
...
{/* 
    ...
    */}
function showBox(id) {
    let discription = UsersObj[id - 1].description
    console.log(id)
    console.log(discription)
    cartBox.innerHTML = `<div class="box1">
            <img src="${UsersObj[id - 1].image}" width="280px"> 
        </div>
        <div class="box2">
            <h2 class='titleProduct'>${UsersObj[id - 1].title}</h2>
            <p>${UsersObj[id - 1].description.slice(0, 120)}</p>
            <p class="price">Prize: $${UsersObj[id - 1].price}</p>
            <p class="rating">Rating: ${UsersObj[id - 1].rating.rate}</p>
            <div class="btn-box">
                <p>Size:</p>
            ...
            <button onclick="cancel()" class="cancel-btn">X</button>`
    let rateBar = document.querySelector('.rating')
    for (let i = 1; i <= UsersObj[id - 1].rating.rate; i++) {
        rateBar.innerHTML += ` <i class="fa-solid fa-star"></i>`
    }
